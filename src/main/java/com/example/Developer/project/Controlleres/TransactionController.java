...
public class TransactionController {
    ...
    public ResponseEntity<Object> getTransaction(@RequestParam String name){
        try{
            Transaction transaction1 = transactionService.getTransaction(name);
    ...
    public ResponseEntity<Object> getTransactionPath(@PathVariable String name){
        try {
            Transaction transaction = transactionService.getTransaction(name);
    ...
    public ResponseEntity<objectResponse> patchTransaction(@RequestBody Transaction transaction_update, @PathVariable String id ){
        ...
        try {
            ...
            return new ResponseEntity<>(new objectResponse("Actualización Exitosa",transaction_bd),HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(new objectResponse(e.getMessage(),null),HttpStatus.INTERNAL_SERVER_ERROR);
        }
